...
How long to wait before emitting events in succession for the same filepath and status. For example if your `Gruntfile_dev.js` file was `changed`, a `changed` event will only fire again after the given milliseconds.
